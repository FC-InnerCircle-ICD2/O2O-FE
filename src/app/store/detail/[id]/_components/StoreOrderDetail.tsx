...
const StoreOrderDetail = () => {
  ...
  const { storeMenuOptions, isSuccess } = useGetStoreMenuOptions(
    orderDetail?.storeId ?? 0,
    orderDetail?.menuId ?? 0
  )
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/50 transition-opacity duration-300">
      ...
      <motion.div
        className="fixed bottom-0 h-[7rem] w-full rounded-tl-lg rounded-tr-lg bg-white px-mobile_safe shadow-[0_-4px_6px_-2px_rgba(0,0,0,0.1)]"
        initial={{
          position: 'absolute',
          borderRadius: '0',
          opacity: 0,
